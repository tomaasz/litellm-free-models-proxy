...
def _post_litellm(path, payload, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            log.warning(
                f"LiteLLM API mutation failed ({e}), retrying in {backoff:.2f}s..."
            )
            time.sleep(backoff)
...
def sync():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        del_futures = {
            executor.submit(delete_model, model_id): (model_str, model_id)
            for model_str, model_id in to_delete
        }
        for future in concurrent.futures.as_completed(del_futures):
            model_str, model_id = del_futures[future]
            if future.result():
        ...
        for future in concurrent.futures.as_completed(add_futures):
            kwargs = add_futures[future]
            if future.result():
                log.info(f"  + {kwargs['model_name']}  ({kwargs['litellm_model']})")
                added += 1
            else:
                errors += 1
